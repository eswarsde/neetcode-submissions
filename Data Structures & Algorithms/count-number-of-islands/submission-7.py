class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        for r in range(ROWS):
            for c in range(COLS):
                if (r, c) not in visted and grid[r][c]== "1": # we only care about land and in this condtiion means, we stumbled upon a new island
                    island_count+=1
                    dfs(r, c) 
                        # dfs function's only job is to update that visted set, which is why it does't return anything. we already indetifed a new island when we check the if condition above
        return island_count
