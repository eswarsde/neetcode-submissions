class Solution:
    def maxProfit(self, prices: List[int]):
        ...
        # assume this as Lowest price seen so far
        buy_price = prices[0]
        # so far no profit
        max_profit_so_far = 0
        
        for price in prices[1:]:
            if price > buy_price: # we can sell and see if we make more profit
                temp_profit = price - buy_price
                max_profit_so_far = max(max_profit_so_far, temp_profit)
            else: # it's better to buy today since this price is lower than previous buy price
                buy_price = price
