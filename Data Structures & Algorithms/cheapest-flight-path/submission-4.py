...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        # # *"In standard Dijkstra, the heap only needs to track Cost and Node to find the cheapest physical route; but when limits are introduced, we must expand our heap state to track Cost, Node, and Stops so we don't accidentally overwrite a slightly more expensive, but legally valid path."*
        ...
        for _ in range(max_flights): # basically BFS for k+1 
